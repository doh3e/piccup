      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <div>
          ...
          <input
            ...
            required
            @blur="validateField(index, 'trainingName')"
          />
          <p v-if="errors[index]?.trainingName" class="text-red-500 text-sm mt-1">
            {{ errors[index].trainingName }}
          </p>
        </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const resetErrors = (index) => {
      ...
      errors.value[index].trainingName = ''
    ...
    const validateField = (index, field) => {
      ...
      if (field === 'trainingName' && (!education.trainingName || education.trainingName.trim() === '')) {
        errors.value[index].trainingName = '교육명은 필수입니다. (Not Blank)'
      }

      if (field === 'startDate' && education.startDate) {
        const startDate = new Date(education.startDate)
        const today = new Date()
        if (startDate > today) {
          errors.value[index].startDate = '교육시작일은 미래일 수 없습니다.'
        }
      }

      if (field === 'endDate' && education.endDate) {
        const endDate = new Date(education.endDate)
        const today = new Date()
        if (endDate > today) {
          errors.value[index].endDate = '교육종료일은 미래일 수 없습니다.'
        }
      }
    }
    ...
    const validateData = () => {
      let isValid = true
      localData.value.forEach((education, index) => {
        validateField(index, 'trainingName')
        validateField(index, 'startDate')
        validateField(index, 'endDate')
        if (errors.value[index]?.trainingName || errors.value[index]?.startDate || errors.value[index]?.endDate) {
          isValid = false
        }
      })
